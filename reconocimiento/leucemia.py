...
for name_sanos in os.listdir(path_sanos):
    ...
    img_ecu = exposure.equalize_adapthist(img_grayS)
    img_gray_ecu = img_grayS + img_ecu
    img_gray_ecu_rest = img_grayS - img_ecu
    img_grayS = img_gray_ecu - img_gray_ecu_rest
    #img_grayS = morphology.closing(img_grayS).astype(np.uint8)
...
for name_leu in os.listdir(path_leucemia):
    ...
    img_ecu = exposure.equalize_adapthist(img_grayL)
    img_gray_ecu = img_grayL + img_ecu
    img_gray_ecu_rest = img_grayL - img_ecu
    img_grayS = img_gray_ecu - img_gray_ecu_rest
    #img_grayL = morphology.closing(img_grayL).astype(np.uint8)
...
#%% incrementalLPCA
lpcaObj = IncrementalPCA(n_components=2, batch_size=2)
LpcaTrain = lpcaObj.fit_transform(img_gray) 
LpcaTrain = LpcaTrain.T 

plt.figure(figsize = (10,24))
plt.scatter(LpcaTrain[0][:480],LpcaTrain[1][:480], color = 'm', label = 'Celulas con Leucemia')
plt.scatter(LpcaTrain[0][480:],LpcaTrain[1][480:], color = 'c', label = 'Celulas Sanas')
plt.xlabel('IPCATrain1')
plt.ylabel('IPCATrain2')
plt.legend()
plt.title('Análisi Incremental de compinentes principales (IPCA) en la matriz de entrenamiento')
plt.show()
